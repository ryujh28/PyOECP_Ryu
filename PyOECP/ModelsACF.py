...
def Discrete(time, par):
    ''' Multiple discrete relaxation model '''

    # time is differ than times
    # times << relaxation times
    # time << time domain
    ...
    for ele in range(len(magnitudes)):
        ...
        if NF is not None:  ## WARNING!!! REVISED!!!
            AF = 2*np.pi*NF
            acf += M * np.exp(-time/T) * np.cos(time * AF)


        #if A is not None and B is not None: ## original
        elif A is not None and B is not None:
            # Havriliak-Negami Model
            print("EORROR Only the Debye relaxation is implemented")
        ...
        for run in tqdm(range(self.production)):
            ...
            if i%100 == 0:
                ...
                if self.AcceptanceCounter_tmp < 20:
                    self.InitialRate[Name][ind] *= 0.95

                    self.AcceptanceCounter_tmp = 0
                elif self.AcceptanceCounter_tmp > 80:
                    self.InitialRate[Name][ind] *= 1.05
